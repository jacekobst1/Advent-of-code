...
class Part01
{
    ...
    private function calculateResult(mixed $file): int
    {
        ...
        while (($line = fgets($file)) !== false) {
            ...
            if ($this->oneSectionFullyContainsTheOtherOne($sections[0], $sections[1])) {
                $result++;
            }
        }
    ...
    private function oneSectionFullyContainsTheOtherOne(array $assignment1, array $assignment2): bool
    {
        $oneFullyContainsTheOther = fn($a, $b) => $a['start'] >= $b['start'] && $a['end'] <= $b['end'];

        return $oneFullyContainsTheOther($assignment1, $assignment2) || $oneFullyContainsTheOther($assignment2, $assignment1);
    }
}
